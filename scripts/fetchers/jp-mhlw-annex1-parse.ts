...
// 각주에 등장하는 성분명(가나/한자) → 한글. 사용자: 일본어 등 다른 언어 노출 금지. 정형 골격
// 한글화 전에 성분명을 먼저 치환(긴 이름 먼저 = 부분겹침 방지).
const JP_SUBSTANCE_KO: [RegExp, string][] = [
  [/グリチルリチン酸/g, "글리시리진산"], [/グリチルレチン酸/g, "글리시레틴산"],
  [/パラアミノ安息香酸/g, "파라아미노안식향산"], [/パラオキシ安息香酸/g, "파라옥시안식향산"],
  [/塩化ラウリルピリジニウム/g, "염화라우릴피리디늄"], [/臭化セチルトリメチルアンモニウム/g, "브롬화세틸트리메틸암모늄"],
  [/エデト酸/g, "에데트산"], [/アラントイン/g, "알란토인"], [/ハッカ油/g, "박하유"],
  [/ｄ１－カンフル/g, "dl-캄포르"], [/ｄ－カンフル/g, "d-캄포르"],
  [/Ｌ－システイン/g, "L-시스테인"], [/へマチン|ヘマチン/g, "헤마틴"],
  [/ショウキョウエキス/g, "생강 추출물"], [/カンタリスチンキ/g, "칸타리스 팅크"],
  [/トコフェロール/g, "토코페롤"], [/安息香酸/g, "안식향산"],
];
function footnoteKo(s: string): string {
  let t = (s || "").trim();
  if (!t) return t;
  for (const [re, ko] of JP_SUBSTANCE_KO) t = t.replace(re, ko);
