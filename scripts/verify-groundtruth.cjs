...
const cleanDisplayName=(raw)=>{const r=(raw||"").toString();if(!r)return r;let s=r.replace(/[\r\n\t]+/g," ").replace(/\s{2,}/g," ").trim();s=s.replace(/\s*\/\s*\d{2,7}-\d{2}-\d.*$/,"").trim();s=s.replace(/\s+and a mixture of\b.*$/i,"").trim();s=s.replace(/[,;]?\s+if (?:they|it) contains?\b.*$/i,"").trim();s=s.replace(/[,;]?\s+except for\b.*$/i,"").trim();s=s.replace(/,(\s*,)+/g,",").replace(/^\s*,\s*|\s*,\s*$/g,"").replace(/\s{2,}/g," ").trim();return s||r;}; // lib 미러
