...
for (const i of ingredients) {
  ...
  if (i.cas_no) for (const cas of i.cas_no.split(/[\s,;/]+/)) { const t = cas.trim().replace(/\(.*$/, ""); if (t) byCas.set(t, i); }  // 슬래시 분리 + 주석 strip(검색/매칭 도달)
}
...
const casTokens = (raw) => raw.split(/[\s,;/]+/).map((c)=>c.trim().replace(/\(.*$/,"")).filter(isValidCas);  // lib 미러: 쉼표/세미콜론/슬래시 분리 + 토큰끝 주석 strip(글루 "68439-49-6(Generic)"·"(cis-)" 등)
...
const cleanDisplayName=(raw)=>{const r=(raw||"").toString();if(!r)return r;let s=r.replace(/[\r\n\t]+/g," ").replace(/\s{2,}/g," ").trim();s=s.replace(/\s*\/\s*\d{2,7}-\d{2}-\d.*$/,"").trim();s=s.replace(/\s*[（(]\s*CAS\s*N[oO]\.?\s*\d{2,7}-\d{2}-\d\s*[）)]/gi,"").trim();s=s.replace(/\s+and a mixture of\b.*$/i,"").trim();s=s.replace(/[,;]?\s+if (?:they|it) contains?\b.*$/i,"").trim();s=s.replace(/[,;]?\s+except for\b.*$/i,"").trim();s=s.replace(/,(\s*,)+/g,",").replace(/^\s*,\s*|\s*,\s*$/g,"").replace(/\s{2,}/g," ").trim();return s||r;}; // lib 미러
function buildCanonical(ids, resolved){
  const members=ids.map(id=>byId.get(id)).filter(Boolean);
  const cleanNm=(v)=>(v==null?v:cleanDisplayName(v)); // lib 미러: 모든 name 필드 균일 위생(한글/중국어/일본어)
  if(members.length<=1){const cleaned=cleanDisplayName(resolved.inci_name);const ko=cleanNm(resolved.korean_name),zh=cleanNm(resolved.chinese_name),ja=cleanNm(resolved.japanese_name);const syn=(resolved.synonyms||[]).filter(x=>!isLeakArtifact(x));const casClean=resolved.cas_no?resolved.cas_no.replace(/[\r\n\t]+/g," ").replace(/\s+,/g,",").replace(/\s{2,}/g," ").trim():resolved.cas_no;return (cleaned===resolved.inci_name&&ko===resolved.korean_name&&zh===resolved.chinese_name&&ja===resolved.japanese_name&&syn.length===(resolved.synonyms||[]).length&&casClean===resolved.cas_no)?resolved:{...resolved,inci_name:cleaned,korean_name:ko,chinese_name:zh,japanese_name:ja,synonyms:syn,cas_no:casClean};}
  ...
  return {id:rep.id,inci_name:repInci,korean_name:cleanNm(firstOf("korean_name")),chinese_name:cleanNm(firstOf("chinese_name")),japanese_name:cleanNm(firstOf("japanese_name")),cas_no:casSet.length?casSet.join(", "):null,synonyms:synSet,description:firstOf("description"),function_category:firstOf("function_category"),function_description:firstOf("function_description"),kcia_code:firstOf("kcia_code")};
}
